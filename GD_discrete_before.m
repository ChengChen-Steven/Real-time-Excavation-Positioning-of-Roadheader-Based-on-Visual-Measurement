...
threshold4=3300;%U*矩阵中threshold3列的积应大于的值，要自己测试得到；50%激光点设计面积（7854）的水平吧；%这个值不简单啊我擦，v2用4000没问题，v4就得3500；
...
R14(R14>0)=1;
count_4=sum(sum(R14))
%IMG3=zeros(2000,3000);IMG(:,:,2)=IMG3;IMG(:,:,3)=IMG3;IMG1=R2;IMG(:,:,1)=IMG1;imshow(IMG)
I
JJ=sum(R14);
...
if n1==N   %要先采用threshold2处理每个矩阵的行和；
                          ...
                          & R2(j+1,(I(i)+1):I(i+1))*u==0 ...
                            & sum(sum(U*R2(j+1:j+threshold3,(I(i)+1):I(i+1))))<threshold4 ...
                              & Sb<threshold4 ...
                                & Sb+sum(sum(U*R2(j+1:j+threshold3,(I(i)+1):I(i+1))))<threshold4;
                                  Sb=0; 
                                  R2(j,(I(i)+1):I(i+1))=zeros(1,(I(i)+1):I(i+1)); %修改了R2 !!!
    ...
    if sum(R2(:,i))>=threshold2%注意这里的threshold2是已经化标为0后的结果；
        R2(:,i)=R2(:,i);
...
%C=roundn(C,-2)
C=[1000.70,600.90,401.30,599.50,1001.08;500.53,999.39,1500.40,2000.45,2500.96];
x_range=[0,v2];
y_range=[0,v1];
x_point=C(2,:);
y_point=C(1,:);
...
L={' 光斑1 (500.53,1000.70)',' 光斑2 (999.39,600.90)',' 光斑3 (1500.40,401.30)',' 光斑4 (2000.45,599.50)',' 光斑5 (2500.96,1001.08)'}; %5个标注
